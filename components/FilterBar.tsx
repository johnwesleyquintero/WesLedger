...
export const FilterBar: React.FC<FilterBarProps> = ({ 
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 mb-6 bg-white p-4 border border-slate-200 rounded shadow-sm items-center">
      ...
      <div className="w-full md:w-auto">
        <input 
          type="month"
          className="block w-full pl-3 pr-3 py-2 text-base border border-slate-300 focus:outline-none focus:ring-slate-500 focus:border-slate-500 sm:text-sm rounded-md bg-slate-50 text-slate-700"
          value={selectedMonth}
      ...
      {(searchQuery || selectedCategory || selectedMonth) && (
         <button
           onClick={() => { setSearchQuery(''); setSelectedCategory(''); setSelectedMonth(''); }}
           className="px-4 py-2 text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded transition-colors whitespace-nowrap"
         >
           Reset Filters
         </button>
      )}
